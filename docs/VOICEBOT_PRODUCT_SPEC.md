...
The flow is implemented as a state machine in `app/agent/orchestrator.py`. State transitions are driven by a lightweight keyword-based intent classifier; Gemini 2.5 Flash handles the natural-language generation within each state.
...
### State 3 - Pitch
- Maps need to product from the Star Health Comprehensive Insurance brochure (family floater, individual cover, senior citizen plan, riders).
...
- "I need to think about it" → "Of course - I can have an advisor call you at a time that works better. What's a good slot?"
- "It's too expensive" → "Premium varies by age and history - the advisor will run the actual quote for your situation. Want me to set that up?"
...
| Retrieval (RAG over brochure) | 50-150 ms | BM25-style retrieval over 203 snippets, dependency-free - sub-10 ms in practice. |
| LLM first-token (Gemini 2.5 Flash) | 300-500 ms | Streaming `chat.completions.create`. |
| TTS first-audio (Bulbul v3) | 200-400 ms | WebSocket streaming; 24 kHz output. |
| **Total time-to-first-audio** | **~750-1300 ms** | Target for production LiveKit streaming path. The current REST-based demo on HuggingFace free tier runs at 7-14s (sequential API round-trips on shared compute). Samvaad benchmark: <500 ms. |
...
- LLM gets `detected_language` in the prompt prefix so Gemini 2.5 Flash responds in the same register.
...
These are the same metrics a Samvaad CSM would walk an insurance carrier through after week 1 of a pilot. Showing them in the demo is the CS signal.
